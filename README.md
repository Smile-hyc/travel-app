...
- 草案采用带时间窗的贪心插入与 `2-opt / relocate / swap / replace` 局部搜索；先把官方确认的父子园区、同票景区和内部接驳解析为游览单元，再执行带容量的地理分区与跨日 `day-move / swap`，避免把同一次游览拆到多天或先平均分景点再补交通。
- 旅行节奏同时控制主要游览单元、参观时长和通勤舒适度：轻松日通常 1～2 个主要单元，适中日目标 3 个，充实日目标 4 个；完整日过空时只补当日开放且不新增超长通勤的顺路候选，到达/离开半天和全天型景区不强制凑数。园区子区域按一个主要游览单元计数，但各自参观和内部接驳时长仍计入当天容量。
...
- 规划方式统一为“智能规划”和“快速规划”：智能规划先生成可执行草案，再依次执行 AI 审稿、有限局部调整和确定性路线复核；快速规划只运行确定性求解器。
- 高德逐段路线核验完成后才调用 DeepSeek。模型只能返回候选 `sourcePoiId` 的局部补丁，不能创建地点或重写整份行程；旧版 `PREFERRED` 请求会兼容映射到智能规划。
- AI 补丁必须重新通过闭馆、餐期、锚点、路线完整性、通勤和强度校验，综合目标至少提升 3% 才会采纳。
- 智能规划期间每 5 秒持续发布等待心跳，不设置额外的服务层硬截止；模型返回后再验收局部补丁，格式不合格或没有量化收益时保留约束草案。
- AI 行程审稿按 DeepSeek 官方协议启用思考模式和 `reasoning_effort=high`，使用独立的较大输出预算判断热门与偏好平衡、体验重复和节奏；结构化局部补丁显式关闭思考模式。若深度审稿没有形成最终 JSON，会再生成一次非思考紧凑审稿；若补丁仍只返回推理或被长度截断，客户端生成可审计的本地空补丁并保留已验收草案，不再误报网关 502。
...
推荐使用城市自动管道，不再手工拼接关键词。默认每城选择 12 个景点，每景点抓取 30 篇候选；通过相关性、广告、重复和城市消歧清洗的笔记全部入库并由详情接口返回。每条体验观察都携带证据 ID，页面可直接打开对应原笔记：
